...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new NoSuchElementException("mostRetweeted on Empty")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    val e = if (p(elem)) l incl elem else l
    right.filterAcc(p, e)
  }
  ...
  def mostRetweeted: Tweet = {
    val l = if(left.isInstanceOf[NonEmpty]) left.mostRetweeted else elem
    val r = if(right.isInstanceOf[NonEmpty]) right.mostRetweeted else elem

    if (elem.retweets >= l.retweets && elem.retweets >= r.retweets) {
      elem
    } else if (l.retweets >= elem.retweets && l.retweets >= r.retweets) {
      l
    } else {
      r
    }
